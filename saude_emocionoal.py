...
if st.button("Finalizar e Gerar Relatório", type="primary"):
    if not st.session_state.respostas:
        ...
        dados_obs = {"Timestamp": [timestamp_str], "Respondente": [respondente], "Observações": [observacoes]}
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            resumo_dimensoes.to_excel(writer, sheet_name='Resultados', index=False)
        ...
        st.download_button(
            ...
            data=processed_data,
            file_name=f"lifenergy_esg_nr1_respostas_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
